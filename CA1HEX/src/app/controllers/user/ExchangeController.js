    ...
    async exchangeByLimit(req, res, next) {
        ...
        if( method === 'buy') {
            ...
            if(market_price > Number(price) ) {
                // If market price is bigger than request price, your request is only added into order book.
                ...
                // If order price is smaller than market price, your request is traded immediately.
                await this.addOrderBook(
                    user._id, 
                    crypto._id, Number(pair_amount) / Number(price), 
                    pair._id, Number(pair_amount) , 
    ...
    async exchangeByNormal(req, res, next) {
        ...
        if(order.type === 'sell') {
            ...
            if(Number(pair_amount) > Number(order.pair_amount)) {
                // 1. Calculate real amount = estimate amount - fee
                ...
                // 4. If you buy crypto of which amount is bigger than order amount.
                await this.fullTradeOrder(orderer_id, next) ;

                // 5. Your rest amount remain in order book.
                await this.addOrderBook(
                    user._id, 
                ...
                // 4. Some crypto amount will be traded, but rest amount still have to remain in order book.
